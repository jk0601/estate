...
차단합니다. 그래서 **전세가는 사람이 입력**해야 전세가율이 계산됩니다.

**방법 1 — 페이지에서 바로 입력(권장, 가족용)**
각 단지 카드의 **"전세·매물 보기"**로 현재 전세 호가를 확인 → 카드 하단
**"전세 최저가 입력"**에 만원 단위로 입력하면 전세가율이 즉시 표시되고
`전세가율 높은순 / 전세 낮은순` 정렬이 활성화됩니다.
- 입력값은 그 **브라우저(기기)에 저장**됩니다(localStorage).
- 다른 가족과 공유하려면 하단 **"전세가 내보내기/가져오기"** 버튼 사용
  (복사한 텍스트를 전달 → 받은 사람이 '가져오기'에 붙여넣기).

**방법 2 — CSV로 일괄 입력(선택)**
`data/jeonse.csv`에 적고 `python collect.py` 재실행 → push 하면 기본값으로 반영됩니다.
```csv
name,jeonse_low_manwon,jeonse_high_manwon,note,updated
반포 래미안 트리니원,180000,,입주장 급전세,2026-07-24
```
(만원 단위: 180000 = 18억. 페이지 입력값이 있으면 그쪽이 우선됩니다.)
